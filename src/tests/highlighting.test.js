...
describe('Exercise Name Validation Tests', () => {
    ...
    function isValidExercise(prefix, fullName) {
        if (!prefix || !fullName)
            return false;
        const variants = Object.keys(exercises_1.EXERCISE_EQUIPMENT);
        const name = fullName.toLowerCase().trim();
        // Try exact match first
        const exactMatch = variants.find(v => v.toLowerCase() === name);
        if (exactMatch && exercises_1.EXERCISE_EQUIPMENT[exactMatch].includes(prefix)) {
            return true;
        }
        ...
        if (partialMatches.length === 1 && exercises_1.EXERCISE_EQUIPMENT[partialMatches[0]].includes(prefix)) {
            return true;
        }
        ...
        for (let i = words.length; i > 0; i--) {
            ...
            if (matchingVariants.length === 1 && exercises_1.EXERCISE_EQUIPMENT[matchingVariants[0]].includes(prefix)) {
                return true;
            }
        }
        return false;
